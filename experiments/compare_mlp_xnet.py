"""MLP vs XNet comparison for the three-branch SVJDM basket solver.

Purpose
-------
Compare the current Triple-Net MLP architecture against the new Triple-Net
XNet architecture under identical Bates / SVJDM dynamics and training settings.

Outputs
-------
  - Summary table  (final Y0, final loss, parameter count, training time)
  - Convergence curves for loss and Y0
  - Saved to figs/compare_mlp_xnet_d50.png
"""
...
from src import solver_triple, solver_xnet
...
def run_comparison():
    """Train MLP and XNet solvers at d=50 with matched settings."""
    print(f"\n{'#'*60}")
    print(f"# MLP vs XNet Comparison — d = {DIM}")
    print(f"{'#'*60}")
...
def plot_comparison(results, save_dir="figs"):
    """Plot loss and Y0 convergence curves for MLP and XNet."""
    ...
    fig.tight_layout()
    path = os.path.join(save_dir, f"compare_mlp_xnet_d{DIM}.png")
...
if __name__ == "__main__":
    ...
    print("\nMLP vs XNet comparison complete.")
