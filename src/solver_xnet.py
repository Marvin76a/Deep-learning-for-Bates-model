"""Three-branch XNet Deep BSDE solver for European basket call under SVJDM.

This mirrors ``solver_triple`` but replaces each MLP branch with an XNet
sub-network based on Cauchy activation functions.
"""
...
from .networks import XNetSubNet
from .utils import plot_training, set_seed


class XNetSolver(nn.Module):
...
def train(cfg: BatesConfig, verbose: bool = True):
    ...
    if verbose:
        print(
            f"=== XNet Basket (SVJDM)  d={cfg.d}  N={cfg.N}  "
...
if __name__ == "__main__":
    ...
    plot_training(losses, y0s, f"(XNet d={cfg.d}, L={cfg.xnet_basis})",
                  save_path=f"figs/xnet_d{cfg.d}_N{cfg.N}_L{cfg.xnet_basis}.png")
